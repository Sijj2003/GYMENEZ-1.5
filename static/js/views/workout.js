...
export const CURRENT_DAY = urlDay ? urlDay.charAt(0).toUpperCase() + urlDay.slice(1).toLowerCase() : 'Lunes';
...
// =======================================================
// 🎬 MODAL DE REPRODUCTOR (YOUTUBE PARSER) 
// =======================================================
...
export async function openTutorial(exerciseName) {
    try {
...
function renderExercises(exercises) {
            ...
            setsRowsHTML += `
                <div id="row-${rowId}" class="grid grid-cols-12 gap-2 items-center bg-black/40 p-2 md:p-3 rounded-xl border border-white/5 transition-all duration-300 text-center">
                    ...
                    <div class="col-span-2 flex justify-center">
                        <button id="btn-${rowId}" data-ex="${exercise.exerciseName.replace(/'/g, "\\'")}" data-set="${s.set_num}" class="confirm-set-btn w-full max-w-[42px] h-8 rounded-lg bg-white/5 text-gray-400 hover:text-white flex items-center justify-center transition font-black text-xs">
                            ✓
                        </button>
                    </div>
                </div>
        ...
        card.innerHTML = `
            <div class="w-full"> 
                ...
                <div class="flex justify-between items-start gap-2 mb-1">
                    ...
                    <button data-tut="${exercise.exerciseName.replace(/'/g, "\\'")}" class="open-tut-btn flex items-center justify-center p-2 rounded-xl border border-white/10 text-gray-300 hover:text-white hover:bg-white/5 transition group flex-shrink-0">
                        <svg class="w-4 h-4 group-hover:text-[#FFC300] transition pointer-events-none" fill="currentColor" viewBox="0 0 20 20"><path d="M4.5 3.5v13L16 10 4.5 3.5z"/></svg>
                    </button>
                </div>
    ...
    // Re-bindear eventos dinámicos
    document.querySelectorAll('.open-tut-btn').forEach(btn => {
        btn.addEventListener('click', (e) => openTutorial(e.currentTarget.dataset.tut));
    });

    document.querySelectorAll('.confirm-set-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const exName = e.currentTarget.dataset.ex;
            const setNum = parseInt(e.currentTarget.dataset.set);
            confirmSetData(exName, setNum);
        });
    });
...
function updateSetRowUI(cardId, setNum, isCompleted) {
    ...
    if (isCompleted) {
        ...
        btn.className = 'confirm-set-btn w-full max-w-[42px] h-8 rounded-lg bg-emerald-500 text-white flex items-center justify-center transition font-black text-xs shadow-[0_0_15px_rgba(16,185,129,0.4)]';
    } else {
        row.classList.remove('border-emerald-500/30', 'bg-emerald-500/5');
        btn.className = 'confirm-set-btn w-full max-w-[42px] h-8 rounded-lg bg-white/5 text-gray-400 hover:text-white flex items-center justify-center transition font-black text-xs';
    }
}
...
// =======================================================
// ⏱️ MOTOR DEL CRONÓMETRO NEURONAL
// =======================================================
let currentRestSeconds = 0;
let totalRestSeconds = 0;

function confirmSetData(exerciseName, setNum) {
    ...
    // 🛡️ BARRERAS DE SEGURIDAD ANALÍTICA
    if (actualWeight < 0) actualWeight = 0;
    if (actualWeight > 1000) { actualWeight = 1000; showMessage("Peso ajustado al límite (Máx 1,000 KG)", "error"); }
    if (actualReps < 0) actualReps = 0;
    if (actualReps > 100) { actualReps = 100; showMessage("Repeticiones ajustadas al límite (Máx 100)", "error"); }
    if (actualRir < 0) actualRir = 0;
    if (actualRir > 4) { actualRir = 4; showMessage("RIR ajustado al rango analítico (0 a 4)", "error"); }
...
if(addTimeBtn) addTimeBtn.addEventListener('click', () => {
    currentRestSeconds += 30; totalRestSeconds += 30; updateTimerUI();
});
...
// =======================================================
// 🚀 INICIALIZACIÓN (Carga de Rutina)
// =======================================================
...
export function initWorkoutView() {
...
window.addEventListener('DOMContentLoaded', initWorkoutView);
...
if (surveyForm) {
    surveyForm.addEventListener('submit', async (e) => {
        ...
        const payload = {
            routine_name: dayTitle.textContent,
        ...
        try {
            ...
            showModalContacto();
        } catch(err) { showMessage("Error al guardar la bitácora.", "error"); }
    });
}
